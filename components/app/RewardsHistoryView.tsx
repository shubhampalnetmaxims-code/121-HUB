...
import { ChevronLeft, Gift, Coins, TrendingUp, TrendingDown, Clock, Info, ShieldCheck, ArrowRight, Lock } from 'lucide-react';
import { User, RewardTransaction, RewardSettings } from '../../types';
...
const RewardsHistoryView: React.FC<RewardsHistoryViewProps> = ({ currentUser, transactions, settings }) => {
    ...
    <div className="h-full flex flex-col bg-slate-50 overflow-hidden text-left relative">
      <div className="bg-white p-6 pt-12 border-b border-slate-100 shrink-0">
        <div className="flex items-center gap-4 mb-2">
          <button onClick={() => navigate('/app/profile')} className="p-2 hover:bg-slate-100 rounded-xl"><ChevronLeft className="w-5 h-5" /></button>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 pb-32 scrollbar-hide">
        ...
        <section className="space-y-4">
           ...
           <div className="space-y-2">
              {userTransactions.length > 0 ? userTransactions.map(tx => (
                <div key={tx.id} className="bg-white p-5 rounded-[28px] border border-slate-100 flex items-center gap-4 group transition-all">
                   ...
                   <div className="flex-1 overflow-hidden">
                      ...
                      <div className="mt-2 text-[8px] font-black text-blue-600 uppercase tracking-widest opacity-60">Balance: {tx.remainingBalance} pts</div>
                   </div>
                </div>
              )) : (
        ...
        <section className="p-6 bg-blue-50 rounded-[32px] border border-blue-100 flex items-start gap-4">
           ...
           <div className="text-left">
              ...
              <p className="text-[10px] text-blue-700 leading-relaxed font-medium">You can redeem points for a discount on any Class, Program, or Marketplace order. Simply toggle 'Use Rewards' during the checkout phase.</p>
           </div>
        </section>
      </div>
