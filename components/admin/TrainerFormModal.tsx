...
import { X, Bold, Italic, List, CloudUpload, User, Mail, Phone, Check } from 'lucide-react';
    ...
    <>
      <div className="fixed inset-0 z-[160] overflow-hidden flex items-center justify-end">
        ...
        <div className="relative h-full w-full max-w-xl bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-500">
          <div className="p-6 md:p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
            <h3 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight">{trainer ? 'Edit Trainer' : 'Add Trainer'}</h3>
          ...
          <form onSubmit={handleFormSubmit} className="flex-1 p-6 md:p-8 space-y-6 overflow-y-auto text-left pb-32 scrollbar-hide">
            ...
            <div className="space-y-3">
              ...
              {formData.facilityIds.length === 0 && <p className="text-[10px] text-red-500 font-bold uppercase">Please select at least one facility.</p>}
            </div>
              ...
              <div className="flex-1 space-y-6">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input required value={formData.name} onChange={e => setFormData(p => ({ ...p, name: e.target.value }))} className="w-full pl-11 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none font-bold" placeholder="e.g. John Doe" />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input required type="email" value={formData.email} onChange={e => setFormData(p => ({ ...p, email: e.target.value }))} className="w-full pl-11 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none font-bold" placeholder="john@121.com" />
                  </div>
                </div>
              </div>
            ...
            <div>
              <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-3">Professional Bio</label>
              <div className="border border-slate-200 rounded-[28px] overflow-hidden bg-slate-50">
                ...
                <div
                  ref={editorRef}
                  contentEditable
                  className="w-full min-h-[150px] p-6 outline-none prose prose-blue max-w-none text-slate-700 text-sm bg-white"
            ...
            <div className="flex flex-col md:flex-row gap-4 pt-10 sticky bottom-0 bg-white/80 backdrop-blur-md pb-4">
              ...
              <button type="submit" disabled={formData.facilityIds.length === 0} className="flex-1 py-4 bg-black text-white rounded-2xl font-extrabold shadow-2xl shadow-black/20 hover:bg-slate-800 transition-all disabled:opacity-50">Save Profile</button>
            </div>
          </form>
        </div>
      </div>
      ...
      {isConfirmingSave && (
        <ConfirmationModal
          ...
          message={`Are you sure you want to save the details for "${formData.name}"?`}
