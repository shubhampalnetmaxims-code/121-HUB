...
import { 
  ...
  ShoppingBag, Users, ArrowLeft, X, CalendarDays
} from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  return (
    <>
      {/* Mobile Overlay */}
      {isOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[140] lg:hidden" onClick={onClose}></div>
      )}

      <aside className={`w-64 bg-black text-white flex flex-col p-8 fixed lg:h-full h-full lg:mt-14 mt-0 shadow-2xl z-[150] transition-transform duration-300 lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
